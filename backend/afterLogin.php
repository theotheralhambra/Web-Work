...
<?php
	...
	$sql = "SELECT empID, password FROM Employee";
	...
	if(mysqli_num_rows($result) > 0) {
	    ...
	    while($row = mysqli_fetch_assoc($result)) {
	        if ($edi == $row['empID'] && $psw == $row['password']) {
	        	$foundUser = true;
	        } 
	    }
	} else { echo "0 results"; }

	if(!$foundUser) { die("Username or password not found!"); }

?>
	
<!-- Save username and password. -->

	 <form method="POST" action="processOption.php" style="width: 400px; margin: 100px auto 0 auto;">
  		<input type="radio" name="adminOption" value="addFilm" checked> Add new movie<br>
  		<input type="radio" name="adminOption" value="deleteFilm"> Delete current movie<br>
 		<input type="radio" name="adminOption" value="addShowtime"> Add showtimes<br>
 		<input type="radio" name="adminOption" value="deleteShowtime"> Delete showtimes<br>
   		<input type="submit">
	</form> 
	
<!-- Print data in the sql -->
<?php 
